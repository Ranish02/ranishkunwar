...
const Instagrampost = () => {
  ...
  useEffect(() => {
    var likes;
    axios
      .get("https://ranish-hub-server.onrender.com/misc/get-like-count")
      .then((res) => {
        console.log(res);
        settotallikes(res.data.message);
      });
  }, []);
  ...
  const sendLikes = () => {
    ...
    //
    axios
      .get("https://ranish-hub-server.onrender.com/misc/add-like")
      .then((res) => {
        settotallikes(res.data.message);
      });
  ...
  const removeLike = () => {
    axios
      .get("https://ranish-hub-server.onrender.com/misc/remove-like")
      .then((res) => {
        settotallikes(res.data.message);
      });
    setisLiked(false);
      ...
      <div className="w-full max-w-[470px] h-full max-h-[680px]  border-x-[1px] border-y-[1px] border-[#9e9e9e] rounded-md ">
        <div className="w-full flex justify-between px-4 my-auto pl-4 h-[47px]">
          <div className="flex my-auto">
            ...
            <div className="ml-1 my-auto text-center">Shrinkhala_Khatiwada</div>
        ...
        <div className="relative content w-full" {...bind}>
          {showlikeanim ? (
            <div
              className={
                isLiked
                  ? "z-20 absolute w-full h-full heart-active heart scale-50"
                  : "z-20 absolute w-[470px] h-[470px] heart scale-50"
              }></div>
          ) : (
            ""
          )}
                ...
                {showlikeanim ? (
                  <div className=" w-[50px]" onClick={handleLike}>
                    <div
                      className={
                        isLiked
                          ? "heart-active h-full w-full heart"
                          : "heart h-full w-full scale-150"
                      }></div>
                  </div>
                ) : (
        ...
        <div className="w-full text-xs flex my-auto pl-4  pt-2">
          <div className="font-bold">Shrinkhala_K20</div>
